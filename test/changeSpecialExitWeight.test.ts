...
test("Should accept and return special exit weight changes", async () => {
  await request(app)
    .post("/change")
    .send({
...
test("Should incorporate special exit weight changes into the map", async () => {
  ...
  await request(app)
    ...
    .expect((res) => {
      ...
      const area: any = map.areas[153];
      const room: any = area.rooms[68];
